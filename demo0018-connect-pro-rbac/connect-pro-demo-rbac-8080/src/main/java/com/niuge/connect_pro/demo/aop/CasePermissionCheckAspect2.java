...
@Component
@Aspect
public class CasePermissionCheckAspect2 {
    ...
    public void doCheck(JoinPoint joinPoint) throws IllegalAccessException {
        ...
        // 获取Controller中的参数
        String caseIdId = obtainCaseId(parameters, dataArray);

        CaseVO caseVO = null;
        // caseVO = caseService.fetchById(caseId);
        if (null == caseVO) {
        ...
        if (!Objects.equals(participantIds, caseVO.participants)) {
            throw new RuntimeException("case不属于当前用户");
        }
    }
